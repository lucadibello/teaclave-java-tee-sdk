...
public class EnclaveFeature implements Feature {
    ...
    private final Map<Class<?>, Boolean> serializationCandidateTypes =
        new HashMap<>();
    private final Map<Class<?>, Boolean> reflectionCandidateTypes =
        new HashMap<>();
    private final Map<Method, Boolean> reflectionCandidateMethods =
        new HashMap<>();
    ...
    public void duringSetup(DuringSetupAccess access) {
        ImageSingletons.add(
            ServiceMethodInvoker.class,
            new ServiceMethodInvoker()
        );
        ImageSingletons.add(LoadServiceInvoker.class, new LoadServiceInvoker());
        ImageSingletons.add(
            UnloadServiceInvoker.class,
            new UnloadServiceInvoker()
        );
        ImageSingletons.lookup(
            RuntimeClassInitializationSupport.class
        ).initializeAtBuildTime(
            "org.apache.teaclave.javasdk.enclave.EnclavePrologue",
            "Prologue class should be initialize at build time."
        );

        FeatureImpl.DuringSetupAccessImpl config =
            (FeatureImpl.DuringSetupAccessImpl) access;
        RuntimeSerialization.register(
            ConfidentialComputingException.class,
            RuntimeException.class,
            ReflectiveOperationException.class,
            ClassNotFoundException.class
        );
        RuntimeSerialization.registerIncludingAssociatedClasses(
    ...
    public void duringAnalysis(DuringAnalysisAccess access) {
        List<Class<?>> enclaveServices = imageClassLoader.findAnnotatedClasses(
            EnclaveService.class,
            true
        );
        enclaveServices.forEach(serviceClazz -> {
            reflectionCandidateTypes.putIfAbsent(serviceClazz, false);
            byte[] serviceConfig = NativeImageResourceFileSystemUtil.getBytes(
                "META-INF/services/" + serviceClazz.getName(),
                true
            );
            try (
                BufferedReader reader = new BufferedReader(
                    new InputStreamReader(
                        new ByteArrayInputStream(serviceConfig),
                ...
                while (true) {
                    ...
                    Class<?> implementation = imageClassLoader
                        .findClass(line)
                        .get();
                    collectConfigs(
                        implementation,
                        Arrays.stream(implementation.getMethods())
                            .filter(method ->
                                serviceClazz.isAssignableFrom(
        ...
        List<Method> extraEnclaveMethods =
            imageClassLoader.findAnnotatedMethods(EnclaveMethod.class);
        extraEnclaveMethods.forEach(method ->
            collectConfigs(method.getDeclaringClass(), List.of(method))
        );
    ...
    public void afterImageWrite(AfterImageWriteAccess access) {
        FeatureImpl.AfterImageWriteAccessImpl a =
            (FeatureImpl.AfterImageWriteAccessImpl) access;
        Path outputDirectory = NativeImageGenerator.generatedFiles(
            a.getUniverse().getBigBang().getOptions()
        );
        Path tempDirectory = ImageSingletons.lookup(
            TemporaryBuildDirectoryProvider.class
        ).getTemporaryBuildDirectory();
        try {
            ...
            Files.walkFileTree(
                ...
                new SimpleFileVisitor<>() {
                    ...
                    public FileVisitResult visitFile(
                        ...
                        if (
                            fileName.endsWith(".o") || fileName.endsWith(".h")
                        ) {
    ...
    private void collectConfigs(Class<?> clazz, List<Method> methods) {
        ...
        methods
            ...
            .forEach(method -> {
                ...
                serializationCandidateTypes.putIfAbsent(
    ...
    private boolean registerCollectedConfigs() {
        boolean registeredNewSerializations = registerCollectedConfigs(
            serializationCandidateTypes,
            RuntimeSerialization::registerIncludingAssociatedClasses
        );
        boolean registeredNewReflectionTypes = registerCollectedConfigs(
            reflectionCandidateTypes,
            RuntimeReflection::register
        );
        boolean registeredNewReflectionMethods = registerCollectedConfigs(
            reflectionCandidateMethods,
            RuntimeReflection::register
        );
        return (
            registeredNewSerializations ||
            registeredNewReflectionTypes ||
            registeredNewReflectionMethods
        );
    }

    private <T> boolean registerCollectedConfigs(
        Map<T, Boolean> configs,
        Consumer<T> registerAction
    ) {
        boolean needRegisterNew = configs
            .entrySet()
            .stream()
